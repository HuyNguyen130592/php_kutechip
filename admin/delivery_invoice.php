<?php
	...
	$id_oder = $_GET['id'];
	if(isset($_GET['oder_status_update']))
		{
			$oder_status_update=$_GET['oder_status_update'];
		}
     ...
     <!-- Main content -->
     	<section class="invoice">
          ...
          <div class="row no-print">
            <div class="col-xs-12">
              <a href="invoice_print.php?id=<?php echo $id_oder ?>" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
              <?php
			  if(isset($_GET['oder_status_update']))
			  {
			  	if($oder_status_update=='OS05')
					{
						echo"<a href='model/oder_status_update.php?id=$id_oder&oder_status_update=$oder_status_update'><button class='btn btn-primary pull-right' style='margin-right: 5px;'><i class='fa  fa-times'></i> Xác nhận hoàn thành giao hàng</button></a>";
					}elseif($oder_status_update=='OS06')
					{
						echo"<a href='model/oder_status_update.php?id=$id_oder&oder_status_update=$oder_status_update'><button class='btn btn-primary pull-right' style='margin-right: 5px;'><i class='fa  fa-times'></i> Xác nhận hủy đơn hàng</button></a>";
					}
             }
			  ?>
            </div>
