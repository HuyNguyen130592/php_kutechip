<!-- jQuery 2.1.3 -->
    ...
    <script type="text/javascript">
		$(document).ready(function(e) {
			...
			$('#pro_price_input_return').maskNumber({integer: true,});
			...
			$('#pro_name_input_return').keyup(function(){
				var c = $('#pro_name_input_return').val();
				...
				$.ajax({
					...
					success: function(e){
						...
						$.each (e, function (key, item){
							html+="<tr>";
							html+="<td><a href='javascript:void(0)' data-price='"+item['price_after']+"' data-name='"+item['name']+"' data-id='"+item['id']+"' class='pro_name_find_return'>"+item['id']+"</a></td>";
							html+="<td><a href='javascript:void(0)' data-price='"+item['price_after']+"' data-name='"+item['name']+"' data-id='"+item['id']+"'  class='pro_name_find_return'>"+item['name']+"</a></td>";
							html+="<td><a href='javascript:void(0)' data-price='"+item['price_after']+"' data-name='"+item['name']+"' data-id='"+item['id']+"' class='pro_name_find_return'>"+item['price']+"</a></td>";
							html+="<td><a href='javascript:void(0)' data-price='"+item['price_after']+"' data-name='"+item['name']+"' data-id='"+item['id']+"' class='pro_name_find_return'>"+item['price_after']+"</a></td>";
							html+="</tr>";
							});
						html+="</tbody>";
						html+="</table>";
						$('#pro_name_return').show();
						$('#pro_name_return').html(html);
						if(Object.keys(e).length==0){$('#pro_name_return').hide();}
						if(c==""||c==" "){$('#pro_name_return').hide();}
						}
					});
			...
			$('#pro_name_return').delegate('.pro_name_find_return','click',function(){
				var id_pro= $(this).attr('data-id');
				var name_pro = $(this).attr('data-name');
				var pro_price = $(this).attr('data-price');
				$('#pro_id_input_return').val(id_pro);
				$('#pro_name_input_return').val(name_pro);
				$('#pro_price_input_return').val(pro_price);
				$('#pro_name_return').hide();
				});
			$('#pro_add_butt').click(function(){
				...
				$.ajax({
					type:"POST",
					url:"model/oder_add_pro_return.php",
					dataType:"json",
			...
			$('#pro_add_butt_return').click(function(){
				
				var id_pro=$('#pro_id_input_return').val();
				var name_pro=$('#pro_name_input_return').val();
				var pro_price=$('#pro_price_input_return').val();
				var pro_quantity = $('#pro_quantity_input_return').val();
				if (pro_quantity == ""){pro_quantity=1}
				$('#pro_id_input_return').val("");
				$('#pro_name_input_return').val("");
				$('#pro_price_input_return').val("");
				$('#pro_quantity_input_return').val("");
				$.ajax({
					type:"POST",
					url:"model/oder_add_pro_returned.php",
					dataType:"json",
					data:{id:id_pro,price:pro_price,quantity:pro_quantity,name:name_pro},
					success: function(e){
						var html ="";
						$.each (e, function (key, item){
							...
							html+="<td>"+item['price']+"</td>";
							html+="<td>"+item['quantity']+"</td>";
							html+="<td><input type='button' data-id='"+item['id']+"' class='btn btn-danger pro_delete_return' value='Xóa' /></td>";
							html+="</tr>";
							
							});
							
						$('#product_tbl_return').append(html);
						}
			...
			$('#product_tbl').delegate('.pro_delete','click',function(){
				...
				$.ajax({
					type:"POST",
					url:"model/oder_product_del_find_return.php",
					dataType:"html",
					data:{id:id},
					success: function(e){}
					});
			...
			$('#product_tbl_return').delegate('.pro_delete_return','click',function(){
				var id = $(this).attr('data-id');
				$(this).parentsUntil('tbody').hide();
				$.ajax({
					type:"POST",
					url:"model/oder_product_del_find_returned.php",
					dataType:"html",
					data:{id:id},
					success: function(e){}
					});
